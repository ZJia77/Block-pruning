...
import math
import lenet
#train the 2*2 block pruning granularity model
...
def draw_connection_layer1_num(model):
    ...
    filename='./layer1_22_weight_0.028.txt'
...
def draw_connection_layer2_num(model):    
    ...
    filename='./layer2_22_weight_0.028.txt'    
    with open(filename,'w') as f:
        for i in range(100):
            for j in range(300):
                f.write(str(t2[i][j])+" ")
...
def draw_connection_layer3_num(model):
    ...
    filename='./layer3_22_weight_0.028.txt'
    with open(filename,'w') as f:
        for i in range(10):
            for j in range(100):
                f.write(str(t3[i][j])+" ")
...
def train_(model1):
    ...
    test_acc=test_model(model1,testloader)
    train_acc=train_model(model1,trainloader)    
    pt.save(model1.state_dict(),"./22_pruning/pruning_10.pt")
    model1.load_state_dict(pt.load("./22_pruning/pruning_10.pt"))
    train_epoch=1
