...
import math
import lenet
#train the 5*5 block pruning granularity model
...
'''
input:
number1, number2: the block pruning granularity(5)
...
def draw_connection_layer1_num(model):
    ...
    filename='./layer1_55_weight_0.028.txt'
...
def draw_connection_layer2_num(model):    
    ...
    filename='./layer2_55_weight_0.028.txt'    
    with open(filename,'w') as f:
        for i in range(100):
            for j in range(300):
                f.write(str(t2[i][j])+" ")
...
def draw_connection_layer3_num(model):
    ...
    filename='./layer3_55_weight_0.028.txt'
    with open(filename,'w') as f:
        for i in range(10):
            for j in range(100):
                f.write(str(t3[i][j])+" ")
...
def train_(model1):
    ...
    test_acc=test_model(model1,testloader)
    train_acc=train_model(model1,trainloader)    
    pt.save(model1.state_dict(),"./55_pruning/pruning_10.pt")
    model1.load_state_dict(pt.load("./55_pruning/pruning_10.pt"))
    train_epoch=1
...
def iterative_pruning_and_training(rounds , number1 , number2):
    ...
    model.load_state_dict(pt.load("./55_pruning/pruning_"+str(rounds)+".pt"))
    ...
    test_acc=test_model(model2,testloader)
    train_acc=train_model(model2,trainloader)    
    pt.save(model2.state_dict(),"./55_pruning/pruning_"+str(rounds+1)+".pt")
    model2.load_state_dict(pt.load("./55_pruning/pruning_"+str(rounds+1)+".pt"))
    train_epoch=1
...
model.load_state_dict(pt.load("./55_pruning/pruning_-1.pt"))
model3=model
...
while rounds<15:
    iterative_pruning_and_training(rounds,5,5)
    rounds=rounds+1
